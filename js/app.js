(function () {
    ...
    function exportNotePdf() {
        ...
        // Flatten resizable wrappers â€” pull inner content out
        bodyClone.querySelectorAll('.resizable-wrapper').forEach((wrapper) => {
            const w = wrapper.style.width || '';
            const h = wrapper.style.height || '';

            // Get the inner content (img, shape, divider)
            const inner = wrapper.firstElementChild;
            if (!inner) { wrapper.remove(); return; }

            // Apply wrapper size to inner element
            if (inner.tagName === 'IMG') {
                inner.style.maxWidth = '100%';
                inner.style.width = w || 'auto';
                inner.style.height = h || 'auto';
                inner.style.objectFit = 'contain';
                inner.style.display = 'block';
                inner.style.margin = '12px 0';
                inner.style.borderRadius = '6px';
            } else if (inner.classList.contains('note-shape-rect')) {
                inner.style.width = w || '60%';
                inner.style.height = h || '80px';
                inner.style.border = '3px solid #232f3e';
                inner.style.borderRadius = '6px';
                inner.style.background = 'rgba(255,153,0,0.06)';
                inner.style.margin = '14px auto';
            ...
            } else if (inner.classList.contains('note-shape-circle')) {
                const size = w || '120px';
                inner.style.width = size;
                inner.style.height = h || size;
                inner.style.border = '3px solid #232f3e';
                ...
                inner.style.color = '#333';
            } else if (inner.classList.contains('note-divider-double')) {
                inner.style.height = '8px';
                inner.style.borderTop = '3px solid #232f3e';
                inner.style.borderBottom = '3px solid #232f3e';
                inner.style.margin = '24px 0';
                inner.style.width = '100%';
            }

            // Replace wrapper with inner content
            wrapper.replaceWith(inner);
        });

        // Style standalone dividers
        bodyClone.querySelectorAll('hr.note-divider').forEach((hr) => {
            hr.style.border = 'none';
            hr.style.borderTop = '3px solid #ff9900';
            hr.style.margin = '20px 0';
        });

        // Style headings
        bodyClone.querySelectorAll('h2').forEach((h) => {
            h.style.color = '#232f3e';
            h.style.fontSize = '20px';
            h.style.borderBottom = '2px solid #ff9900';
            h.style.paddingBottom = '6px';
            h.style.marginTop = '20px';
        });

        // Style blockquotes
        bodyClone.querySelectorAll('blockquote').forEach((bq) => {
            bq.style.borderLeft = '4px solid #ff9900';
